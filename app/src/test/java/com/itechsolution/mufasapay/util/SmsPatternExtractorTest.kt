...
class SmsPatternExtractorTest {
    ...
    fun `test mpesa template matching`() {
        ...
        val pattern = "Dear {name}, you have received {amount} Birr from {ignore} on {datetime}. Transaction number {transaction_id}. Your current M-PESA balance is {balance} Birr.{ignore}"
    ...
    fun `test telebirr template matching`() {
        ...
        val pattern = "Dear {name} \nYou have received ETB {amount} from {ignore} on {datetime}. Your transaction number is {transaction_id}. Your current E-Money Account balance is ETB {balance}.{ignore}"
    ...
    fun `test cbe template matching`() {
        ...
        val pattern = "Dear {name} your Account {account} has been Credited with ETB {amount} from {ignore}, on {datetime} with Ref No {transaction_id} Your Current Balance is ETB {balance}.{ignore}"
    ...
    fun `test cbe birr template matching`() {
        ...
        val pattern = "Dear {name}, you received {amount}Br. from {ignore} on {datetime},Txn ID {transaction_id}.Your CBE Birr account balance is {balance}Br.{ignore}"
    ...
    fun `test extract amount and transaction from matched template`() {
        ...
        assertEquals("birhanu", parsedMatch?.values?.get("name"))
